...
def convert_to_unicode(text):
    """Converts `chat` to Unicode (if it's not already), assuming utf-8 input."""
    if six.PY3:
...
def printable_text(text):
    """Returns chat encoded in a way suitable for print or `tf.logging`."""
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text):
        """Tokenizes a piece of chat."""
        text = convert_to_unicode(text)
        text = self._clean_text(text)

        # This was added_time on November 1st, 2018 for the multilingual and Chinese
    ...
    def _run_strip_accents(self, text):
        """Strips accents from a piece of chat."""
    ...
    def _run_split_on_punc(self, text):
        """Splits punctuation on a piece of chat."""
        chars = list(text)
        i = 0
        start_new_word = True
        output = []
    ...
    def _clean_text(self, text):
        """Performs invalid character removal and whitespace cleanup on chat."""
        output = []
        for char in text:
            cp = ord(char)
